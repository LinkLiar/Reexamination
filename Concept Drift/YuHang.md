...
模仿人类做决策的过程
特征标签的熵即为条件熵
标签的熵与特征标签的熵之差即是该特征信息增益（反映这一维特征的重要程度）
目录：
1.决策树学习策略：以损失函数为目标函数的最小化
2.特征选择：递归地选择最优特征，并根据该特征对训练数据进行分剖，使得对各个子数据集有一个最好的分类
3.决策树生成：与特征选择过程相对应，选择的特征依次形成决策树的结点，直至所有训练数据子集都被基本正确分类，或无合适特征可选
4.决策树的剪枝：为避免过拟合，对已生成的树自下而上进行剪枝，将树变得更简单，从而使它具有更好的泛化能力

信息增益最大的类别作为根节点
决策树的划分依据1：信息增益，信息增益率，基尼值和基尼指数，
CART Gini系数：可分类和回归，离散或连续数据都可
ID3 信息增益：只能对离散属性的数据集
C4.5 信息增益率：解决ID3分支过程中总喜欢偏向取值较多的属性
...
同时，无论是ID3, C4.5还是CART,在做特征选择的时候都是选择最优的一个特征来做分类决策。
但大多数分类决策不应该是由某一个特征决定的，而应由一组特征决定的。这样决策得到的决策树更加准确。这个决策树叫做多变量决策树(multi-variate decision tree)。在选择最优特征的时候，多变量决策树不是选择某一个最优特征，而是选择最优的一个特征线性组合来做决策。这个算法的代表是OC1。
