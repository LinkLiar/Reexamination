...
MergeFrameResult GetUnionOfStrings(MergeFrameResult mergedResult1, MergeFrameResult mergedResult2)
{
	...
	for (int i = 0, x = 1; i < static_cast<int>(result.size()); i++, x++)
	{
		...
		{
			int headGap = result[0].first > result[0].second ? result[0].second : result[0].first;    //min
			int headBiasGap = result[0].first < result[0].second ? result[0].second : result[0].first;    //max

			vector<pair<int, string> >tempString;
			if (headBiasGap - headGap == 0)
			{
			...
			else
			{
				...
				if (headGap != 0)
				{
					for (int j = 0; j < headGap; j++)
					{
						if (j == 0)
							vote[0].second.push_back(make_pair(0, add));
						vote[0].second.back().second.push_back(str2[result[0].first - headGap + j]);
						count.push_back(tempOdds[result[0].first - headGap + j + 1].first + '0');
					}
					vote[0].second.back().first = atoi(count.c_str());
				}
			...
			else
			{
				...
				if (middleGap != 0)
				{
					for (int j = 0; j < middleGap; j++)
					{
						if (j == 0)
							vote[x - 1].second.push_back(make_pair(0, add));
						vote[x - 1].second.back().second.push_back(str2[result[i].first - middleGap + j]);
	...
	for (; iterOdds != vote.end() && (iterOdds->first!=0 || !iterOdds->second.empty()); ++iterOdds)
	{
		...
		if (!iterOdds->second.empty())
		{
			...
			if (adds.size() >= 1)
			{
				...
				if (adds.size() != 0)
				{
					final.first = to_string(main.first);
					final.second = main.second;
					final.second.erase(final.second.begin());
